...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content):
        ...
        try:
            if command == "get_general_notifications":
                ...
                if payload == None:
                    raise ClientError(205,"accept friend don't work correctly")
                else:
                    payload = json.loads(payload)
                ...
                if payload == None:
                    raise ClientError(206, "decline friend don't work correctly")
                else:
                    payload = json.loads(payload)
                    await self.send_updated_friend_request_notification(payload['notification'])
                ...
                if payload==None:
                    raise ClientError(210,"Refresh general_notifications failed")
                else:
                    payload = json.loads(payload)
    ...
    async def send_updated_friend_request_notification(self, notification):
        ...
        """
        await self.send_json({
            "general_msg_type": GENERAL_MSG_TYPE_UPDATE_NOTIFICATION,
    ...
    async def general_pagination_exhausted(self):
        ...
        await self.send_json({
            "general_msg_type":GENERAL_MSG_TYPE_PAGINAGION_EXHAUSTED,
...
def get_general_notifications(user, page_number):
    ...
    if user.is_authenticated:
        ...
        payload = {}
        print("number pages:"+str(p.num_pages))
        ...
        raise ClientError(204,"You must be authenticated")
...
def accept_friend_request(user, notification_id):
    ...
    if user.is_authenticated:
        try:
            ...
            friend_request = notifications.content_object
            ...
            raise ClientError(422,"An error occured with that notification")
        return None
...
def refresh_general_notifications(user, oldest_timestamp, newest_timestamp):
    ...
    if user.is_authenticated:
        ...
        newest_ts = oldest_timestamp[0:newest_timestamp.find("+")]
        ...
        #oldest_ts = datetime.strptime(''.join(oldest_timestamp.rsplit(':', 1)),'%Y-%m-%d %H:%M:%S.%f%z')


        #newest_ts = datetime.strptime(''.join(newest_timestamp.rsplit(':', 1)), '%Y-%m-%d %H:%M:%S.%f%z')
...
def get_new_general_notifications(user, newest_timestamp):
    ...
    payload={}
    if user.is_authenticated:
        # timestamp = datetime.strptime(''.join(newest_timestamp.rsplit(':', 1)), '%Y-%m-%d %H:%M:%S.%f%z')
